...
закоммиченного `proof_stats.json` по `htmlSha256` и 21 PNG sha256. `capture`
запускается вручную через `workflow_dispatch` и еженедельно по расписанию
(`0 4 * * 1`). Если будущий runner даст иные пиксельные хэши, job падает; канон
пересобирается из артефактов того прогона (identity принадлежит рецепту, см.
§1), а расхождение фиксируется здесь, не замалчивается.
